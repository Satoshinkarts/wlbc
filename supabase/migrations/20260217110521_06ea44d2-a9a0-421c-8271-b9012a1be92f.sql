...
-- Drop the old overloaded version without promo_code
DROP FUNCTION IF EXISTS public.validate_and_create_order(jsonb, text, text);

-- Replace the function with payment_proof_path parameter
CREATE OR REPLACE FUNCTION public.validate_and_create_order(
  _items jsonb,
  _shipping_address text DEFAULT ''::text,
  _notes text DEFAULT ''::text,
  _promo_code text DEFAULT NULL::text,
  _payment_proof_path text DEFAULT NULL::text
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
